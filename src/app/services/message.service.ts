...
export class MessageService {
  ...
  sendMessage(query: { email: string; asunto: string; mensaje: string; telefono: string; }) {
  ...
  sendMessageTas(query: { email: string; nombre: string; mensaje: string; telefono: string; }) {

    let base = 'https://us-central1-node-app-372ef.cloudfunctions.net/sendMailTas?';
    let url = base + 'email=' + query.email + '&nombre=' + query.nombre + '&mensaje=' + query.mensaje + '&telefono=' + query.telefono ; 
    // '&mensaje=' + query.mensaje + '&consultat=' + query.consultat + '&telefono=' + query.telefono;
    //let _body = 'email=germanwibaux@gmail.com&mensaje=hola queria consulasknda akjsbdnaius bnialsu nbolauis dnbpuiasnd bpaiosud &asunto=presupuestar mi casa';
